...
class DQNPrioritizedReplay:
    def __init__(
        ...
        self.memory = Memory(capacity=self.memory_size) if self.prioritized else np.zeros((self.memory_size, n_features * 2 + 2))
        self.loss_hist = []
        self.build_network()
    ...
    def build_network(self):
        ...
        target_fc1.bias.data.normal_(0.1, 0)
    ...
    def learn(self):
        ...
        if self.prioritized:
            abs_errors = torch.abs(q_eval - q_target)
            loss_val = self.loss_func(q_eval, q_target)
            loss_val = loss_val * torch.FloatTensor(ISWeights).cuda()
            self.memory.batch_update(tree_idx, abs_errors.cpu().data.numpy())
        else:
            loss_val = self.loss_func(q_eval, q_target)
        ...
        self.loss_hist.append(loss_val.mean().cpu().data.numpy())
